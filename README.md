...
[View the live demo on Heroku](https://react-redux-boilerplate-101.herokuapp.com/) or the [individual UI components](https://ivanarielcaceres.github.io/bootcamp-gdg-frontend) of my Storybook on Github Pages.
...
- Download this boilerplate repository as a zip:
  https://github.com/ivanarielcaceres/bootcamp-gdg-frontend/archive/master.zip
...
```
  git init
  git remote add origin https://github.com/ivanarielcaceres/$REPOSITORY_NAME.git
```

- Replace `bootcamp-gdg-frontend` with `$REPOSITORY_NAME` and `ivanarielcaceres` with `$YOUR_GITHUB_USERNAME` on `package.json`
- Replace `bootcamp-gdg-frontend` with `$REPOSITORY_NAME` on `README.md` (so the links will not be broken)
...
- Setup the Github env variables on Travis CI. Go to
  https://travis-ci.org/ivanarielcaceres/$REPOSITORY_NAME and add the env variable `GH_TOKEN`
...
```
travis encrypt MY_HEROKU_AUTH_TOKEN -r ivanarielcaceres/bootcamp-gdg-frontend
```
